...
function createFile(root, item, templates) {
  ...
  if( item.metadata && item.metadata.template && templates[item.metadata.template] ) {
    template = templates[item.metadata.template];
  } else if( item.template ) {
    console.log('Unable to find template: '+item.template+'.  Known templates: '+Object.keys(templates));
  }
